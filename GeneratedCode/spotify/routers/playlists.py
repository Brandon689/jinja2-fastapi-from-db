...
from models import Playlists, PlaylistsCreate
...
@router.get("/", response_model=List[Playlists])
...
@router.get("/{item_id}", response_model=Playlists)
async def get_playlists_item(item_id: int):
...
@router.post("/", response_model=Playlists)
async def create_playlists(item: PlaylistsCreate):
...
@router.put("/{item_id}", response_model=Playlists)
async def update_playlists(item_id: int, item: PlaylistsCreate):
    set_clause = ", ".join(f"{k} = ?" for k in item.dict().keys())
    query = f"UPDATE playlists SET {set_clause} WHERE playlist_id = ?"
...
async def delete_playlists(item_id: int):
